  ...
  <div>
    ...
    <CCard>
      ...
      <CCardBody>
        ...
        <div v-if="verify == 'reject'">
          <br />
          <CTextarea label="เนื่องจาก" v-model="form.ISM_approve_reason" />
        </div>
      </CCardBody>
    </CCard>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        ISM_approve_reason: "",
        ISM_date: new Date(),
        process_name: "ISMR Approve",
      },

      verify: "approve",

      verify_options: [
        { value: "approve", label: "เห็นชอบ" },
        { value: "reject", label: "ไม่เห็นชอบ" },
      ],
  ...
  watch: {
    verify: function (val) {
      ...
      this.form.ISM_approve = this.verify_options[index].label;

      if (val == "approve") {
        this.form.ISM_approve_reason = "";
      }
    },
  },
};
