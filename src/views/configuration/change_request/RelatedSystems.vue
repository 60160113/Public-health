<template>
  <div>
    <CCard>
      <CCardHeader>
        <strong class="text-primary">ระบบที่เกี่ยวข้อง</strong>
      </CCardHeader>
      <CCardBody>
        <CButton
          color="primary"
    ...
    >
      ...
      <CTextarea v-else label="ระบบที่เกี่ยวข้อง" v-model="form.system" />
      <template #header>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "system", label: "ระบบที่เกี่ยวข้อง", _style: "width:70%" },
  ...
  methods: {
    getList() {
      ...
      this.jogetListAll("mophApp", "list_change_request_related_systems")
        .then((res) => {
          this.list = res.data;
          this.loading = false;
        })
    ...
    submit(primaryKey, formData) {
      return this.jogetFormSubmit(
        "mophApp",
        "change_request_related_systems",
        primaryKey,
        formData
      );
    },
    ...
    remove() {
      this.loading = true;
      this.jogetFormDelete("mophApp", "change_request_related_systems", this.form.id)
        .then((res) => {
          this.getList();
          this.modal = false;
        })
