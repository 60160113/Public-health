<template>
  <div>
    <CCard>
      <CCardHeader>
        <strong class="text-primary">เหตุผลที่ขอเปลี่ยนแปลง</strong>
      </CCardHeader>
      <CCardBody>
        <CButton
          color="primary"
    ...
    >
      ...
      <CTextarea v-else label="เหตุผลที่ขอเปลี่ยนแปลง" v-model="form.reason" />
      <template #header>
        <h6 class="modal-title">
          {{
            status == "remove" ? "ลบ" : status == "add" ? "เพิ่ม" : "แก้ไข"
          }}เหตุผลที่ขอเปลี่ยนแปลง
        </h6>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "reason", label: "เหตุผลที่ขอเปลี่ยนแปลง", _style: "width:70%" },
  ...
  methods: {
    getList() {
      ...
      this.jogetListAll("mophApp", "list_change_request_reasons")
        .then((res) => {
          this.list = res.data;
          this.loading = false;
        })
    ...
    remove() {
      this.loading = true;
      this.jogetFormDelete("mophApp", "change_request_reasons", this.form.id)
        .then((res) => {
          this.getList();
          this.modal = false;
        })
